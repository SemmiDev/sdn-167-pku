<x-admin-layout>
    <div class="flex justify-between">
        <h1 class="text-3xl text-black pb-6">Data Jenis kegiatan</h1>
        <div>
            <button data-modal-target="tambah-atribut-modal" data-modal-toggle="tambah-atribut-modal"
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <tbody>
                @foreach ($daftarAtribut as $atribut)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4 text-center">
                            <button data-id="{{ $atribut->id }}" data-nama="{{ $atribut->nama }}"
                                data-keterangan="{{ $atribut->keterangan }}" data-modal-target="edit-atribut-modal"
                            ...
                            <form action="{{ route('app.atribut.destroy', $atribut->id) }}" method="post"
                                class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
        ...
        <div id="tambah-atribut-modal" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative w-full max-w-md max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <button type="button"
                        ...
                        data-modal-hide="tambah-atribut-modal">
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Masukkan Data Jenis kegiatan</h3>
                        <form class="space-y-6" action="{{ route('app.atribut.store') }}" method="post">
                            @csrf
                            <div>
                                <label for="nama"
                                ...
                                {{ $errors->has('nama') ? 'border-red-500' : '' }}"
                                    placeholder="Sholat Berjamaah" required>
                            </div>
                            ...
                            <div>
                                <label for="komponen"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Komponen Kegiatan</label>
        ...
        <div id="edit-atribut-modal" tabindex="-1" aria-hidden="true"
            class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative w-full max-w-md max-h-full">
                <!-- Modal content -->
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <button type="button"
                        class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                        data-modal-hide="edit-atribut-modal">
                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 14 14">
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Edit Data Jenis kegiatan</h3>
                        <form id="edit-atribut-form" class="space-y-6" method="post">
                            @csrf
                            @method('PUT')
                            <div>
                                <label for="nama_edit"
    ...
    <script type="text/javascript">
        $('.edit-atribut').click(function() {
            ...
            var url = "{{ route('app.atribut.update', ':atribut') }}";
