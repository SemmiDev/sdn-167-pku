<x-admin-layout>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <tbody>
                @foreach ($daftarGuru as $guru)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4 text-center">
                            <button data-id="{{ $guru->id }}" data-nama="{{ $guru->nama }}"
                                data-email="{{ $guru->email }}" data-jenis_kelamin="{{ $guru->jenis_kelamin }}"
                                data-alamat="{{ $guru->alamat }}" data-no_telepon="{{ $guru->no_telepon }}"
                            ...
                            <form action="{{ route('app.guru.destroy', $guru->id) }}" method="post"
                                class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
            ...
            <div class="relative w-full max-w-md max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        ...
                        <form class="space-y-6" action="{{ route('app.guru.store') }}" method="post">
                            @csrf
                            <div>
                                <label for="nama"
                            ...
                            <div>
                                ...
                                {{ $errors->has('email') ? 'border-red-500' : '' }}"
                                    placeholder="susanti@gmail.com" required>
                            </div>
        ...
        <div id="edit-guru-modal" tabindex="-1" aria-hidden="true"
            class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative w-full max-w-md max-h-full">
                <!-- Modal content -->
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        ...
                        <form id="edit-guru-form" class="space-y-6" method="post">
                            @csrf
                            @method('PUT')
                            <div>
                                <label for="nama_edit"
                            ...
                            <div>
                                <label for="no_telepon_edit"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">No
                                    Telepon</label>
                                <input type="text" name="no_telepon" id="no_telepon_edit"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white
                                {{ $errors->has('no_telepon') ? 'border-red-500' : '' }}"
                                    placeholder="08xxxxxxxxxx" required>
                            </div>
    ...
    <script type="text/javascript">
        $('.edit-guru').click(function() {
            ...
            var url = "{{ route('app.guru.update', ':guru') }}";
