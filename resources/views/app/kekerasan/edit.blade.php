<x-admin-layout>
    <form class="space-y-6 max-w-4xl mx-auto"
        action="{{ route('app.kekerasan.update', ['kekerasan' => $kekerasan->id]) }}" method="post">
        @csrf
        @method('PUT')
        <div>
            ...
            <select name="id_siswa" id="siswa"
                ...
                @foreach ($daftarSiswa as $siswa)
                    <option value="{{ $siswa->id }}" {{ $siswa->id == $kekerasan->id_siswa ? 'selected' : '' }}>
                        {{ $siswa->nama }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select name="id_atribut" id="atribut"
                ...
                @foreach ($daftarAtribut as $atribut)
                    <option value="{{ $atribut->id }}" {{ $atribut->id == $kekerasan->id_atribut ? 'selected' : '' }}>
        ...
        <div>
            ...
            <textarea name="keterangan" id="keterangan"
                ...
                placeholder="Winner memukul kepala temannya">{{ $kekerasan->keterangan }}</textarea>
        </div>
