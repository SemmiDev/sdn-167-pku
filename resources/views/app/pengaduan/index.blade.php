        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            @foreach ($daftarPengaduan as $pengaduan)
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <th scope="row"
                        class="px-6 flex justify-center py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white text-center">
                    ...
                    <td class="px-6 py-4 text-center">
                        ...
                        @endphp
                        {{ \Carbon\Carbon::parse($pengaduan->tanggal_kejadian)->isoFormat('dddd, D MMMM YYYY') }}
                        Jam {{ \Carbon\Carbon::parse($pengaduan->jam_kejadian)->format('H:i:s') }}
                    </td>

                    <td class="px-6 py-4 text-center">
                        <div class="flex gap-2 justify-center items-center">
                            <a href="{{route('app.pengaduan.edit', $pengaduan->id)}}"
                               class="w-20 focus:outline-none text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-900">
                                Edit
                            </a>
                            @role('OPERATOR_SEKOLAH')
                            <form action="{{ route('app.pengaduan.destroy', $pengaduan->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
