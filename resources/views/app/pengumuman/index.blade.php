<x-admin-layout>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <tbody>
                @foreach ($daftarPengumuman as $pengumuman)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4">
                            <button data-id="{{ $pengumuman->id }}" data-judul="{{ $pengumuman->judul }}"
                                data-keterangan="{{ $pengumuman->keterangan }}"
                                data-modal-target="edit-pengumuman-modal" data-modal-toggle="edit-pengumuman-modal"
                            ...
                            <form action="{{ route('app.pengumuman.destroy', $pengumuman->id) }}" method="post"
                                class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
            ...
            <div class="relative w-full max-w-md max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <button type="button"
                        ...
                        data-modal-hide="tambah-pengumuman-modal">
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        ...
                        <form class="space-y-6" action="{{ route('app.pengumuman.store') }}" method="post">
                            @csrf
                            <div>
                                <label for="judul"
                                ...
                                <input type="text" name="judul" id="judul"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white
                                {{ $errors->has('judul') ? 'border-red-500' : '' }}"
                                    placeholder="Rapat Wali Murid" required>
                            </div>

                            <div>
                                <label for="text"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Keterangan</label>
                                <textarea name="keterangan" cols="30" rows="10" id="keterangan"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white
                                {{ $errors->has('keterangan') ? 'border-red-500' : '' }}"
                                    placeholder="Diharapkan semua wali murid ..." required></textarea>
                            </div>
        ...
        <div id="edit-pengumuman-modal" tabindex="-1" aria-hidden="true"
            class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative w-full max-w-md max-h-full">
                <!-- Modal content -->
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <button type="button"
                        class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                        data-modal-hide="edit-pengumuman-modal">
                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 14 14">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        ...
                        <form id="edit-pengumuman-form" class="space-y-6" method="post">
                            @csrf
                            @method('PUT')
                            <div>
                            ...
                            <div>
                                <label for="keterangan_edit"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Keterangan</label>
                                <textarea cols="30" rows="10" name="keterangan" id="keterangan_edit"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white
                                {{ $errors->has('keterangan') ? 'border-red-500' : '' }}"
                                    required></textarea>
                            </div>
    ...
    <script type="text/javascript">
        $('.edit-pengumuman').click(function() {
            ...
            var url = "{{ route('app.pengumuman.update', ':pengumuman') }}";
